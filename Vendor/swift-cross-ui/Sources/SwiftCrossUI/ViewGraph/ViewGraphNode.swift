...
public class ViewGraphNode<NodeView: View, Backend: BaseAppBackend>: Sendable {
        ...
        environment: EnvironmentValues)
    {
        ...
        self.resultCache = [:]
        self.lastProposedSize = .zero
        self.parentEnvironment = environment
        self.cancellables = []

        self.dynamicPropertyUpdater = DynamicPropertyUpdater(for: nodeView)

        let viewEnvironment = self.updateEnvironment(environment)

        self.dynamicPropertyUpdater.update(self.view, with: viewEnvironment, previousValue: nil)
        ...
        forEachField(of: self.view) { name, _, fieldValue in
            #if DEBUG
    ...
    private func bottomUpUpdate() {
        ...
        let currentSize = self.currentLayout?.size
        let newLayout = self.computeLayout(
            proposedSize: self.lastProposedSize,
            environment: self.parentEnvironment)

        self.currentLayout = newLayout
        if newLayout.size != currentSize {
            self.resultCache[self.lastProposedSize] = newLayout
            self.parentEnvironment.onResize(newLayout.size)
        } else {
            _ = self.commit()
        }
    }
        ...
        environment: EnvironmentValues) -> ViewLayoutResult
    {
        ...
        if !self.hasHadFirstUpdate {
            // We show the widget here instead of in init, because in init the widget
            // hasn't been added to its parent widget yet.
            self.backend.show(widget: self.widget)
            self.hasHadFirstUpdate = true
        }

        if proposedSize == self.lastProposedSize, !self.resultCache.isEmpty,
           !self.parentEnvironment.allowLayoutCaching || environment.allowLayoutCaching,
           let currentLayout
        {
        ...
        self.parentEnvironment = environment
        self.lastProposedSize = proposedSize
        ...
        self.dynamicPropertyUpdater.update(self.view, with: viewEnvironment, previousValue: previousView)

        let result = self.view.computeLayout(
            self.widget,
        ...
        self.resultCache[proposedSize] = result
    ...
    public func commit() -> ViewLayoutResult {
        ...
        if self.parentEnvironment.allowLayoutCaching {
            logger.warning(
        ...
        if currentLayout.size.height == .infinity || currentLayout.size.width == .infinity {
            logger.warning(
                ...
                metadata: [
                    ...
                    "lastProposedSize": "\(self.lastProposedSize)",
                ])
        }
        ...
        self.backend.showUpdate(of: self.widget)

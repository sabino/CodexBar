...
final class WindowReference<SceneType: WindowingScene> {
    ...
    /// Content-derived size limits are invariant during a pure external resize.
    private var cachedContentSizeLimits: (minimum: ViewSize, maximum: ViewSize?)?
        ...
        id: String)
    {
        ...
        let rootWidget = self.viewGraph.rootNode.concreteNode(for: Backend.self).widget
        ...
        environment: EnvironmentValues)
    {
        ...
        if self.isFirstUpdate, isProgramaticallyResizable, !backend.restoresWindowFrames {
            proposedWindowSize = environment.defaultWindowSize
            usedDefaultSize = true
        } else {
            proposedWindowSize = self.cachedWindowSize ?? backend.size(ofWindow: window)
        ...
        windowSizeIsFinal: Bool = false)
    {
        ...
        let sizeLimits: (minimum: ViewSize, maximum: ViewSize?)
        if newScene == nil, let cachedContentSizeLimits {
            sizeLimits = cachedContentSizeLimits
        } else {
            let probingResult = self.viewGraph.computeLayout(
            ...
            case .contentSize:
                let result = self.viewGraph.computeLayout(
                    with: newScene?.content(),
            ...
            sizeLimits = (minimumWindowSize, maximumWindowSize)
            cachedContentSizeLimits = sizeLimits
        }
        let minimumWindowSize = sizeLimits.minimum
        let maximumWindowSize = sizeLimits.maximum
        ...
        let finalContentResult = self.viewGraph.computeLayout(
        ...
        self.cachedWindowSize = proposedWindowSize
